...
baseurl=$AW_REPO_URL/rpm/dependencies/$(grep version group_vars/all | grep -o '[[:digit:]].[[:digit:]]')/epel-$1-x86_64
enabled=0
gpgcheck=$GPGCHECK
    ...
    case $(distribution_id) in
        rhel|centos|ol)
            ...
            if [ $? -eq 0 ]; then
                ...
                tmprepo=
                if [ ! -f /etc/yum.repos.d/ansible-tower.repo ]; then
                    case ${DISTRIBUTION_MAJOR_VERSION} in
                        7)
                ...
                yum install -y --enablerepo='ansible-tower-dependencies' ansible
...
if [ $? -eq 0 ]; then
    ...
    if [ "$(distribution_id)" == "centos" ] || [ "$(distribution_id)" == "rhel" ] || [ "$(distribution_id)" == "ol" ]; then
        ...
        ANSIBLE_RPM_PATH=$(find_bundle_rpm "ansible*.el${DISTRIBUTION_MAJOR_VERSION}ae.*noarch.rpm")
